...
BuildTopology(boost::json::object& configObj)
{
    ...
    // std::map<Ipv4Address, DcTopology::TopoNode> mp;
    ...
    //         for (int j = 0; j < naddr; j++)
    //         {
    //             // NS_LOG_DEBUG(ipv4->GetAddress(i, j).GetAddress());
    //             mp[ipv4->GetAddress(i, j).GetAddress()] = node;
    //         }
    //     }
    // }

    // do some routing tests
         ...
         hostSrc++)
    {
        ...
        Ipv4RoutingTableEntry entry = route->GetRoute(0);
        Ipv4Address swAddr = entry.GetGateway();

        //     Ptr<Ipv4ListRouting> lrouting =
        //         DynamicCast<Ipv4ListRouting>((*hostSrc)->GetObject<Ipv4>()->GetRoutingProtocol());
        //     int16_t prio;

        //     Ptr<Ipv4GlobalRouting> glb =
        //         DynamicCast<Ipv4GlobalRouting>(lrouting->GetRoutingProtocol(1, prio));
        //     Ptr<Ipv4StaticRouting> route =
        //         DynamicCast<Ipv4StaticRouting>(lrouting->GetRoutingProtocol(0, prio));
        //     // auto x = route->GetRoute(0).GetDest();
        //     NS_ASSERT(route);
        //     NS_ASSERT(route->GetNRoutes() > 0);
        //     for (uint32_t i = 0; i < route->GetNRoutes(); i++)
        //     {
        //         NS_LOG_DEBUG("route " << i << " " << route->GetRoute(i));
        //     }
        //     for (uint32_t i = 0; i < glb->GetNRoutes(); i++)
        //     {
        //         NS_LOG_DEBUG("glb " << i << " " << glb->GetRoute(i)->GetDest());
        //     }
        // traverse hostDst
             ...
             hostDst++)
        {
            ...
            // Ptr<Ipv4Route> found =
            //     route->RouteOutput(packet, ipHdr, (*hostSrc)->GetDevice(1), theerrno);
            // Ptr<Ipv4Route> found =
            //     route->RouteOutput(packet, ipHdr, (*hostSrc)->GetDevice(1), theerrno);
            Ptr<Ipv4Route> found = route->RouteOutput(packet, ipHdr, 0, theerrno);
            NS_LOG_DEBUG("hostSrc " << (*hostSrc)->GetId() << " hostDst " << (*hostDst)->GetId()
                                    << " dstAddr " << dstAddr << " found " << *found);
            ...
            for (uint32_t i = 0; i < channel->GetNDevices(); i++)
            {
                ...
                if (dev != found->GetOutputDevice())
                {
                    NS_LOG_DEBUG("dev " << i << " " << dev->GetNode()->GetId());
                    Ptr<Ipv4GlobalRouting> newRoute =
                        dev->GetNode()->GetObject<GlobalRouter>()->GetRoutingProtocol();
                    // traverse every other device on the node, and find the routeoutput
                    for (uint32_t j = 0; j < dev->GetNode()->GetNDevices(); j++)
                    {
                        Ptr<NetDevice> dev2 = dev->GetNode()->GetDevice(j);
                        if (dev2 != dev)
                        {
                            Ptr<Ipv4Route> found2 =
                                newRoute->RouteOutput(packet, ipHdr, 0, theerrno);
                            if (theerrno == Socket::ERROR_NOTERROR)
                                NS_LOG_DEBUG("dev2 " << j << " " << dev2 << " " << *found2);
                        }
                    }

...
ConfigureSwitch (boost::json::object& configObj, Ptr<Node> sw)
{
  ...
  boost::json::object switchConfigObj = configObj["topologyConfig"].get_object ()
    .find ("switch")->value ().get_object ();
  ...
  DcbSwitchStackHelper switchStack;
  switchStack.SetBufferSize (QueueSize (std::string (switchConfigObj.find ("bufferSize")->value ().as_string ().c_str ())));
  switchStack.InstallPortsProtos(sw);

  // Configure the port and its queues according to the config object
  boost::json::object switchPortConfigObj = configObj["topologyConfig"].get_object ()
    .find ("switchPort")->value ().get_object ();
  boost::json::object switchPortQueueConfigObj = configObj["topologyConfig"].get_object ()
    .find ("switchPortQueue")->value ().get_object ();
  ...
  for (uint32_t portIdx = 1; portIdx < sw->GetNDevices (); portIdx++)
    {
      ...
      if (pfcEnabled)
        {
          std::string sPfcReserve (switchPortQueueConfigObj.find ("pfcReserve")->value ().as_string ().c_str ());
          std::string sPfcXon (switchPortQueueConfigObj.find ("pfcXon")->value ().as_string ().c_str ());
      ...
      if (ecnEnabled)
        {
          std::string sEcnKMin (switchPortQueueConfigObj.find ("ecnKMin")->value ().as_string ().c_str ());
          std::string sEcnKMax (switchPortQueueConfigObj.find ("ecnKMax")->value ().as_string ().c_str ());
          double ecnPMax = switchPortQueueConfigObj.find ("ecnPMax")->value ().as_double ();
          std::string sSwitchBufferSize = configObj["topologyConfig"].get_object ()
            .find ("switch")->value ().get_object ()
            .find ("bufferSize")->value ().get_string ().c_str ();
...
{
  boost::json::object linkConfigObj = configObj["topologyConfig"].get_object ()
    .find ("link")->value ().get_object ();
...
{
  std::string topoFilename = configObj["inputFile"].get_object ()
    .find ("topo")->value ().as_string ().c_str ();
  std::ifstream topof;
